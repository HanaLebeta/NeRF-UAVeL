...
def parse_args():
    ...
    parser.add_argument('--macblock_placement', type=str, default='C5',
                        help='MACBlock (TACA) placement: C5, all, none')
    parser.add_argument('--taca_placement', type=str, default='all',
                        help='TACA placement: C2,C3,C4,C5,C4C5,all,none')
    parser.add_argument('--use_maxpool', action='store_true',
                        help='Use MaxPool3d instead of MVAP')
    parser.add_argument('--mvap_kernel_size', type=int, default=5,
                        help='MVAP initial conv kernel size: 3,5,7,9')
    parser.add_argument('--ddaf_dilations', type=str, default='1,2,4',
                        help='DDAF dilation rates as comma-separated: e.g. 1,2,4')
    parser.add_argument('--vcwaf_window_size', type=str, default='2,2,2',
                        help='V-CWAF window size as comma-separated: e.g. 2,2,2')
    parser.add_argument('--vcwaf_num_heads', type=int, default=4,
                        help='V-CWAF number of attention heads: 1,2,4,8')
...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        if args.checkpoint:
            ...
            self.model.backbone.load_state_dict(checkpoint['backbone_state_dict'], strict=False)
    ...
    def build_backbone(self):
        if self.args.backbone_type == 'resnet':
            ...
            ddaf_dilations = tuple(int(x) for x in self.args.ddaf_dilations.split(','))
            vcwaf_ws = tuple(int(x) for x in self.args.vcwaf_window_size.split(','))
            self.backbone = VGG_FPN(
                "EF", self.args.input_dim, True, self.args.resolution,
                taca_placement=self.args.taca_placement,
                mvap_kernel_size=self.args.mvap_kernel_size,
                use_maxpool=getattr(self.args, 'use_maxpool', False),
                ddaf_dilations=ddaf_dilations,
                vcwaf_window_size=vcwaf_ws,
                vcwaf_num_heads=self.args.vcwaf_num_heads,
                macblock_placement=self.args.macblock_placement,
            )
    ...
    def train_loop(self):
        ...
        for epoch in range(1, self.args.num_epochs + 1):
            ...
            if epoch % self.args.eval_interval == 0 or epoch == self.args.num_epochs:
                ...
                # Save dense checkpoints every 10 epochs between 600-800
                if 600 <= epoch <= 800 and epoch % 10 == 0:
                    ckpt_dir = os.path.join(self.args.save_path, 'checkpoints_600_800')
                    os.makedirs(ckpt_dir, exist_ok=True)
                    self.save_checkpoint(epoch, os.path.join(ckpt_dir, f'epoch_{epoch}.pt'))
                    self.logger.info(f'Saved dense checkpoint at epoch {epoch}')

                # Save last epoch
                if epoch == self.args.num_epochs:
                    self.save_checkpoint(epoch, os.path.join(self.args.save_path, 'model_last.pt'))
